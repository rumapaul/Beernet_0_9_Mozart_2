...
%%             Partition script with checking of number of consecutive nodes on a partition
%%             sequential partition script, partition of almost equal size
%%             sparsed partition script, partition of almost equal size 
...
[BeerConstants]={Module.link ["../commons/Constants.ozf"]}
...
thread
  ...
  proc{RemovePartitionedLink S T}
   ...
   proc {CollectSparseEdges N}
      proc {CollectRouletteRound Round FNodes}
          if Round \= N then
                CurrentNodes = {LoopNetworkRoulette @MasterOfPuppets @FNodes N+1-Round}
                FNodes := {List.append @FNodes CurrentNodes}
                RestNodes = {ListMinus @AlivePbeers @FNodes}%Remove only CurrentNodes 4 both-way failure
                in
                {ForAll CurrentNodes 
                 ...
                 {CollectRouletteRound Round+1 FNodes}
          end
      end
      FromNodes = {NewCell nil}
      in
      {CollectRouletteRound 1 FromNodes}
   end

   proc{DoPartition N Choice}
      if Choice == 1 then
         {CollectSparseEdges N}
      else
...
in
   ...
   {O onEnter(proc{$ E}
                 ...
                 {String.token E &: F P}
                 case F
		 of "SequentialPartition" then
                    N = {String.toInt P}
                    if N>1 andthen N=<@NetworkSize andthen
                        @NetworkSize div N < BeerConstants.slSize then
                        {DoPartition N 0}
                    else
                        skip
                    end
                 [] "SparsePartition" then
                    N = {String.toInt P}
                    if N>1 andthen N=<@NetworkSize then
                        {DoPartition N 1}
                    else
                        skip
                    end
...
fun {LoopNetworkRoulette Master NodeBasket Probability}
      proc {Loop Current Result ConsecutiveCollection}
         if Current \= nil andthen
	    Current.id \= {Master getId($)} then
            Succ = {ComLayer sendTo(Current getSucc($))}
            RNum = {OS.rand} mod Probability 
            in
            if {Not {List.member Current NodeBasket}} andthen
               RNum == Probability - 1 andthen 
               @ConsecutiveCollection < BeerConstants.slSize - 1 then
                Result := Current|@Result
                ConsecutiveCollection := @ConsecutiveCollection + 1
            else
                ConsecutiveCollection := 0
            end 
	    {Loop Succ Result ConsecutiveCollection}
         end
      end
      Result
      NoConsecutive
   in
      Result = {NewCell nil}
      NoConsecutive = {NewCell 0}
      {Loop {Master getSucc($)} Result NoConsecutive}
      @Result
   end
